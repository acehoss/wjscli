...
| --- | --- | --- | --- | --- |
| `wiki_pages_tree` | `pages.tree` | — | `parent` (0), `mode` ('ALL'), `locale` ('en'), `depth` (1) | `mode` is one of `'ALL' \| 'PAGES' \| 'FOLDERS'`. Returns `{ tree: [...] }` — flat list with `depth` (absolute from wiki root) and `parent`. With `depth > 1`, the tool recursively queries `pages.tree(parent=child.id)` for each entry at the level above, in parallel (Promise.all), and concatenates the results DFS-order (parent then its subtree). Each extra level adds one GraphQL round-trip per node at the level above. |
