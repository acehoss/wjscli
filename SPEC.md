...
| --- | --- | --- | --- | --- |
| `wiki_pages_tree` | `pages.tree` | — | `parent` (0), `mode` ('ALL'), `locale` ('en'), `depth` (20) | `mode` is one of `'ALL' \| 'PAGES' \| 'FOLDERS'`. Returns `{ tree: [...] }` — flat list with `depth` (absolute from wiki root) and `parent`. The tool recursively queries `pages.tree(parent=child.id)` for each entry at the level above, in parallel (Promise.all), and concatenates the results DFS-order (parent then its subtree). The default `depth: 20` is "effectively unlimited for normal wikis"; agents can pass a smaller number on very large wikis. Each level adds one GraphQL round-trip per node at the level above. |
