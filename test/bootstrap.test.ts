...
beforeEach(async () => {
  tmpRoot = await fs.mkdtemp(path.join(tmpdir(), 'wikijs-mcp-boot-'));
...
describe('runBootstrap — usage errors', () => {
  it('returns 2 with no args', async () => {
    expect(await runBootstrap([])).toBe(2);
  });

  it('returns 2 with one arg', async () => {
    expect(await runBootstrap(['https://wiki.example.com'])).toBe(2);
  });

  it('returns 2 on an invalid URL', async () => {
    expect(await runBootstrap(['not a url', validJwt])).toBe(2);
  });

  it('returns 2 on a non-http(s) scheme', async () => {
    expect(await runBootstrap(['ftp://wiki.example.com', validJwt])).toBe(2);
  });

  describe('JWT shape rejection', () => {
    it('rejects a single-segment string', async () => {
      expect(await runBootstrap([mock.url, 'not-a-jwt'])).toBe(2);
    });

    it('rejects an empty string', async () => {
      expect(await runBootstrap([mock.url, ''])).toBe(2);
    });

    it('rejects a two-segment value', async () => {
      expect(await runBootstrap([mock.url, 'aaaa.bbbb'])).toBe(2);
    });

    it('rejects a value with a trailing empty segment', async () => {
      expect(await runBootstrap([mock.url, 'aaaa.bbbb.cccc.'])).toBe(2);
    });

    it('rejects a value with a non-base64url character (+)', async () => {
      expect(await runBootstrap([mock.url, 'aaaa.bbb+b.cccc'])).toBe(2);
    });
  });
});
...
describe('runBootstrap — happy path', () => {
  it('writes config with the supplied JWT (no refresh) and exits 0', async () => {
    ...
    const code = await runBootstrap([mock.url, validJwt]);
    expect(code).toBe(0);
  ...
  it('sends the provided JWT in the Authorization header', async () => {
    await runBootstrap([mock.url, validJwt]);
  ...
  it('canonicalizes the base URL before persisting', async () => {
    ...
    const code = await runBootstrap([withSlash, validJwt]);
    expect(code).toBe(0);
    const cfg = await readConfig(mock.url);
    expect(cfg?.baseUrl).toBe(mock.url);
  });
...
describe('runBootstrap — failure paths', () => {
  ...
  it('does NOT classify a permission "not authorized" message as auth failure', async () => {
    ...
    // would falsely tell the user to re-bootstrap when their JWT is fine.
    mock.setNext({
